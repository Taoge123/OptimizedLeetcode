...
class Solution1:
    def minRefuelStops(self, target, startFuel, stations):
        ...
        now = startFuel
        count = 0
        while target > now:  # out loop, we add gas once a time.
            while stations and stations[0][0] <= now:  # memorize every possibility
                heapq.heappush(heap, -stations.pop(0)[1])
            if not heap: return -1  # no fuel, still haven't get to target
            now -= heapq.heappop(
                heap)  # greedily add the maximum, just once!so that we are able to check other more possibilities.
            count += 1
        return count
